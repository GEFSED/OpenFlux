...
func runCaptchaSteps(t *testing.T, steps []captchaStep, wantError string) *volgaAuth {
	t.Helper()
	client := newVolgaAuthClient(MobileVolgaConfig())
	...
	a, err := authorizeWithClient(context.Background(), fixtureDoc, MobileVolgaConfig(), client)
...
func TestCaptchaReadAndRedirectBounds(t *testing.T) {
	...
	t.Run("normal_mobile_oversize_bound", func(t *testing.T) {
		s := normalSteps()[:1]
		s[0].body = strings.Repeat("x", (2<<20)+1)
		runCaptchaSteps(t, s, "too large")
	})
}
...
func TestCaptchaPrivacy(t *testing.T) {
	...
	utils.SetDebug(true)
	defer func() { utils.SetDebug(false); utils.SetOutput(os.Stderr) }()
	...
	runCaptchaSteps(t, s, "captcha request failed")
	if strings.Contains(logs.String(), "SENTINEL") || strings.Contains(logs.String(), "https://") || strings.Contains(logs.String(), "12345") {
...
// injected DialContext, including challenge GET/POST and the original retry.
func TestCaptchaIOSDialerAndNoFallback(t *testing.T) {
	...
	limits := MobileVolgaConfig()
	limits.DialContext = func(ctx context.Context, network, address string) (net.Conn, error) {
		dials.Add(1)
	...
	client := newVolgaAuthClient(limits)
	...
	defer client.CloseIdleConnections()
	if _, e := authorizeWithClient(context.Background(), fixtureDoc, MobileVolgaConfig(), client); e != nil {
		t.Fatal(e)
	}
	...
	dials.Store(0)
	limits.DialContext = func(context.Context, string, string) (net.Conn, error) {
		dials.Add(1)
		return nil, errors.New("SENTINEL_DIAL")
	}
	client2 := newVolgaAuthClient(limits)
...
func TestCaptchaStopDuringAuthorization(t *testing.T) {
	...
	tn := NewYandexVolgaTransport(fixtureDoc, cfg, MobileVolgaConfig())
	done := make(chan error, 1)
	go func() { done <- tn.Start() }()
	select {
	case <-entered:
	...
	for range 4 {
		stops.Add(1)
		go func() { defer stops.Done(); _ = tn.Stop() }()
	}
	stops.Wait()
	select {
	case e := <-done:
	...
	case <-time.After(time.Second):
		t.Fatal("Stop did not cancel authorization")
	}
	if e := tn.Start(); e == nil {
		t.Fatal("Start after Stop accepted")
	}
	if dials.Load() != 1 || tn.relay != nil || tn.ws != nil || tn.IsRunning() {
		t.Fatal("workers/start survived Stop")
	}
}
