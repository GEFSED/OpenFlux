...
func localGuardRelay(t *testing.T, enabled bool, handler func(http.ResponseWriter, *http.Request, int64)) (*configuredRelayClient, *rateLimitTestClock, *atomic.Int64) {
	t.Helper()
	count := &atomic.Int64{}
	...
	r := newConfiguredRelayClient(&volgaAuth{Session: &http.Client{}, Token: "synthetic-token", RequestPath: "synthetic-path"}, cfg, &configuredVolgaStats{})
	t.Cleanup(r.Stop)
	r.httpClient.Transport = localRoundTripper(func(req *http.Request) (*http.Response, error) {
...
func guardBatch(r *configuredRelayClient) error { return r.sendBatch([][]byte{[]byte("synthetic-packet")}) }

func asyncGuardBatch(r *configuredRelayClient) <-chan error {
...
func Test429GuardPerformanceSnapshot(t *testing.T) {
	...
	expectGuardResult(t, guardBatch(r), true)
	tr := newConfiguredVolgaTransport("synthetic-document", transport.DefaultConfig(), r.config)
