...
func TestVolgaDefaultsUnchanged(t *testing.T) {
	...
	if _, found := reflect.TypeOf(configuredRelayClient{}).FieldByName("queue"); found {
...
func TestBase64BoundedAndWireIdentical(t *testing.T) {
	for _, n := range []int{0, 1, 200, 1400, 8192, 65535, 4 << 20} {
		...
		if configuredBase64Encode(p) != base64.StdEncoding.EncodeToString(p) {
	...
	fresh := configuredB64Pool.New().([]byte)
	if cap(fresh) != 4096 {
...
func TestRelaySendStop100Cycles(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		r := newConfiguredRelayClient(&volgaAuth{Session: &http.Client{Jar: jar}}, cfg, &configuredVolgaStats{})
