...
- **Экспериментальный wire v3** - выключен по умолчанию: batched отправляет v2
  без служебных записей. `OPENFLUX_EXPERIMENTAL_WIRE_V3=1` на обоих peers
  включает прототип только для изолированных тестов. Handshake не аутентифицирован
  и не привязан к сессии; защиты от replay и безопасного reconnect пока нет.
...
- L3 отбрасывает фрагментированные IPv4-датаграммы. MTU 1280 не предотвращает
  фрагментацию крупных UDP-датаграмм приложения. Reassembly и передача ICMP/PMTU
  не реализованы.
- Linux raw L3 UDP требует резервирования/трансляции исходных портов: raw socket
  не мешает ядру отправлять ICMP port-unreachable для незанятого UDP-порта.
  Конфликты с портами хоста также не решены. Для UDP используйте L4; L3 нельзя
  считать готовым до устранения этих проблем и проверки на Linux.
...
остаётся v2. Обратная совместимость и reconnect экспериментального v3 не
подтверждены; не включайте его на недоверенных каналах.
