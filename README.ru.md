...
- **IPv4 UDP** - DNS, QUIC и другой UDP-трафик поддерживаются обоими
  exit-бэкендами. SOCKS5-клиенты используют RFC 1928 `UDP ASSOCIATE`.
- **Согласуемый wire v3** - новые batched-клиенты объявляют возможности внутри
  безопасной для старых версий v2-записи и переходят на v3 только после
  обнаружения второго v3-peer. V3-кадры содержат session ID, sequence number и
  проверяемую длину payload. Со старым batched-peer соединение остаётся v2.
...
- Фрагментированные IPv4-датаграммы отбрасываются: в не-первом фрагменте нет
  портов, необходимых conntrack. До реализации PMTU/reassembly оставьте MTU
  туннеля равным 1280.
...
**Важно:** batched и legacy LZ4 по-прежнему несовместимы. Внутри batched-кодека
согласование v3 обратно совместимо с v2 и автоматически остаётся на v2, если
peer не объявляет поддержку v3.
