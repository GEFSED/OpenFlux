...
// Baseline runs the Android startup/callback/queue path, replacing only Volga.
// Other profiles run the same wrapper and finishStart used by startClient.
...
func TestProductionV060LegacyCompatibility(t *testing.T) {
	...
	for _, profile := range []string{"baseline", "balanced", "low_latency", "throughput", "throughput_current", "throughput_mem", "throughput_96"} {
		for _, encrypted := range []bool{true, false} {
			...
			t.Run(profile+"/"+label, func(t *testing.T) {
				...
				baselineActive.Store(profile == "baseline")
				t.Cleanup(func() {
					Stop()
					baselineActive.Store(false)
				})
				if profile == "baseline" {
					if err := baselineStartWithCarrier("vyandex", productionTestContext, secret, "legacy", "", "", a); err != "" {
						t.Fatal(err)
					}
					aes = baselineClient.encrypted
				} else {
					session = newPacketSession(profile, "vyandex", 8)
					installSession(session)
					tr, err := wrapPacketTransport(session, a, productionTestContext, secret, "legacy", true)
					if err != nil {
						t.Fatal(err)
					}
				...
				if encrypted {
					...
					if d.Success != 2 || d.BadHeader != 0 || d.DecryptFail != 0 {
						t.Fatalf("client AES receive: %+v", d)
					}
				}
				if profile == "baseline" {
					if baselineClient.callbackPackets != 2 || baselineClient.enqueued != 2 {
						t.Fatal("baseline callback/enqueue counts")
					}
				} else if session.callbackPackets != 2 || session.enqueued != 2 {
