...
func TestModeExactConfigs(t *testing.T) {
	...
	if speed.volga.RateLimit429GuardEnabled || !optimized.volga.RateLimit429GuardEnabled {
		t.Fatal("guard enable")
	}
	optimized.volga.RateLimit429GuardEnabled = false
	if !reflect.DeepEqual(speed, optimized) {
		t.Fatal("more than the guard enable differs")
	}
	standard := modeConfig("standard")
	if standard.volga != yandex.DefaultVolgaConfig() || standard.outer != transport.DefaultBatchedConfig() {
		t.Fatal("Standard default changed")
	}
	for _, name := range []string{"", "standard", "unknown", "throughput_w32", "throughput_w32_429guard"} {
		if normalizeMode(name) != "standard" || modeConfig(name) != standard {
			t.Fatal("default/fallback changed")
		}
	}
	for _, name := range []string{"standard", "speed", "optimized"} {
		if normalizeMode(name) != name {
			t.Fatal("mode name changed")
		}
		for _, kind := range []string{"", "yandex", "oneme", "mailru", "cupsonline"} {
			if effectiveMode(kind, name) != "standard" {
				t.Fatal("non-Volga scheduling changed")
			}
		}
	}
}
...
func TestModeDiagnosticsAreSafe(t *testing.T) {
	configureLogging()
	if utils.IsVerbose() {
		t.Fatal("provider debug logging enabled")
	}
	ReadLogs()
	for _, mode := range []string{"speed", "optimized"} {
		...
		for _, denied := range []string{"synthetic-private", "http://", "https://", "Cookie", "Bearer"} {
			if strings.Contains(got, denied) {
				t.Fatal("private data in summary")
			}
		}
		s.stop()
	}
...
func TestStopDuringStart(t *testing.T) {
	...
	case <-time.After(time.Second):
		t.Fatal("reader not cancelled")
	}
	close(p.release)
	if <-finished == "" {
		t.Fatal("cancelled startup published")
	}
	<-p.stopped
	if IsConnected() {
		t.Fatal("stopped session became connected")
	}
	installSession(nil)
}
